  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.zsh = {
      siteFunctions.fzf-goto-widget = /* bash */ ''
        _goto_resolve_db
        local dir="$(${lib.getExe pkgs.gnused} 's/ /:/' $GOTO_DB | column -t -s : | FZF_DEFAULT_OPTS="$FZF_DEFAULT_OPTS $FZF_GOTO_OPTS" ${lib.getExe config.programs.fzf.package} | ${lib.getExe pkgs.gnused} "s/^[a-zA-Z]* *//")"
        if [[ -z "$dir" ]]; then
          zle redisplay
          return 0
        fi
